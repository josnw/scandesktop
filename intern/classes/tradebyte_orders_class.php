		...
		while ( $line = $this->importHandle->readCSV() ) {
			...
			foreach($row as $key=>$value) {
				...
				if (substr($key,0,4) == 'POS_') {
					$this->OrdersData[$row['TB_ORDER_ID']]['pos'][$row['POS_LFDNR']][$key] = preg_replace('/[\x00-\x1F\x80-\xA0]/u', '', $value);
				} else {
					$this->OrdersData[$row['TB_ORDER_ID']]['head'][$key] = preg_replace('/[\x00-\x1F\x80-\xA0]/u', '', $value);
...
//			'FBLG' => $this->OrdersData[$orderId]['head']['TB_ORDER_ID'],
			'FBLG' =>  $this->channel[$this->OrdersData[$orderId]['head']['CHANNEL_KEY']]['Filiale'] * 10000000 + $this->OrdersData[$orderId]['head']['TB_ORDER_ID'],
